 ...
 *   Unset:  dot(['a', 'b', 'c'], { a: { b: { c: 1 } } }, undefined, true)
 */
function dot (keys = [], obj = {}, value = _null, unset = _null) {
         let _apply = function (keys, obj, value) {
             if (keys.length == 1 && (value !== _null || unset !== _null)) {
                return unset !== _null ? delete obj[keys[0]] : obj[keys[0]] = value.apply(this, [obj[keys[0]] || _null]);
              } else {
...
module.exports = {
    _null: _null,
    dotNotation: (keys = _null, obj = {}, value = _null, unset = _null) => {
        ...
        return Array.isArray(obj) ? obj.map((o, idx) => {
                   ...
                   return dot(String(Array.isArray(keys) ? keys[idx] : keys).trim().split('.'), o, value, unset);
               }) : dot((keys !== _null ? String(keys).trim().split('.') : []), obj, value, unset);
    },
    is: (val, type) => {
